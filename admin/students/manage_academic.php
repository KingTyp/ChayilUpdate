...
if(isset($_GET['id'])){
    ...
    if($qry->num_rows > 0){
        ...
        echo "<center><small class='text-muted'>Unkown Customer ID.</small</center>";
        exit;
    }
}
?>
    ...
    <form action="" id="academic-form">
        ...
        <div class="row">
            <div class="col-md-6 form-group">
                <label for="semester" class="control-label">Property In Need</label>
                <select name="semester" id="semester" class="form-control form-control-sm  form-control-border rounded-0" required>
                    <option <?= !isset($semester) || (isset($semester) && $semester == 'First Semester') ? 'selected' : '' ?>>Warehouse</option>
                    <option <?= (isset($semester) && $semester == 'Second Semester') ? 'selected' : '' ?>>Land</option>
                    <option <?= (isset($semester) && $semester == 'Third Semester') ? 'selected' : '' ?>>Residential</option>
                </select>
            </div>
            <div class="form-group col-md-6">
                <label for="school_year" class="control-label">Contact</label>
        ...
        <div class="row">
            <div class="col-md-6 form-group">
                <label for="course_id" class="control-label">Person In Charge</label>
            ...
            <div class="form-group col-md-6">
                <label for="year" class="control-label">Region</label>
                <input type="text" id="year" name ="year" value="<?= isset($year) ? $year : '' ?>" class="form-control form-control-border form-control-sm" required>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 form-group">
                <label for="status" class="control-label">Feedback Status</label>
                <select name="status" class="form-control form-control-sm form-control-border rounded-0" required>
                    <option value="1" <?= (isset($status) && $status == 1) ? 'selected' : '' ?>>Yes,Liked</option>
                    <option value="2" <?= (isset($status) && $status == 2) ? 'selected' : '' ?>>No,Didn't like</option>
                    <option value="3" <?= (isset($status) && $status == 3) ? 'selected' : '' ?>>Inspection Done</option>
                    <option value="4" <?= (isset($status) && $status == 4) ? 'selected' : '' ?>>Inspection Not Done</option>
                </select>
            </div>
            <div class="col-md-6 form-group">
                <label for="end_status" class="control-label">Transaction Status</label>
                <select name="end_status" class="form-control form-control-sm form-control-border rounded-0" required>
                    ...
                    <option value="1" <?= (isset($end_status) && $end_status == 1) ? 'selected' : '' ?>>Paid</option>
                    <option value="2" <?= (isset($end_status) && $end_status == 2) ? 'selected' : '' ?>>Cancelled</option>
                </select>
            </div>
        </div>
    </form>
